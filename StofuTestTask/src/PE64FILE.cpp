...
PEResult PE64FILE::LoadFromFile(int8_t* m_loadedPeFilePtr) {
    UnloadFile();
...
PEResult PE64FILE::SaveToFile(std::filesystem::path filePath)
{
    Commit();
    RebuildImportTable();
    ...
    file.write((char*)&m_ntHeaders, sizeof(IMAGE_NT_HEADERS64));
...
int32_t PE64FILE::AddSection(std::string_view name, DWORD size, bool isExecutable)
{
...
void PE64FILE::AddImport(std::string_view dllName, char** functions, int functionCount)
{
...
void PE64FILE::PrintImportTable()
{
...
PEResult PE64FILE::ReadHeaders()
{
    ...
    // Read PE headers
    memcpy(&m_ntHeaders, m_loadedPeFile + m_dosHeader.e_lfanew, sizeof(IMAGE_NT_HEADERS64));
    ...
    memcpy(m_sectionTable, m_loadedPeFile + m_dosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS64), m_ntHeaders.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER));

    return PEResult::Success;
}

void PE64FILE::ReadSections()
{
    DWORD reservedDataOffset = m_dosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS64) + m_ntHeaders.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER);
...
PEResult PE64FILE::ReadImports()
{
    ...
    while (true)
    {
        ...
        {
            importThunk = (IMAGE_THUNK_DATA64*)(m_loadedPeFile + RvaToOffset(importDesc->OriginalFirstThunk));
        }
        else
        {
            importThunk = (IMAGE_THUNK_DATA64*)(m_loadedPeFile + RvaToOffset(importDesc->FirstThunk));
        }
        ...
        while (true)
        {
            ...
            {
                importFunction->Id = IMAGE_ORDINAL64(importThunk->u1.Ordinal);
            }
            else
            {
            ...
            importThunk = (IMAGE_THUNK_DATA64*)((char*)importThunk + sizeof(IMAGE_THUNK_DATA64));
...
void PE64FILE::RebuildImportTable()
{
...
char* PE64FILE::BuildAdditionalImports(DWORD baseRVA)
{
    Commit();
    ...
    while (importDll != nullptr)
    {
        ...
        while (importFunction != nullptr)
        {
            ...
            if (importFunction->Id != 0)
            {
                importThunk.u1.Ordinal = importFunction->Id | IMAGE_ORDINAL_FLAG64;
            }
            else
            {
...
DWORD PE64FILE::CalculateAdditionalImportsSize(DWORD& sizeDlls, DWORD& sizeFunctions, DWORD& sizeStrings)
{
    PE_IMPORT_DLL_ENTRY* importDll = &this->m_additionalImports;
    PE_IMPORT_FUNCTION_ENTRY* importFunction;
...
bool PE64FILE::WritePadding(std::ofstream& file, long paddingSize)
{
    if (paddingSize <= 0)
        return false;
...
DWORD PE64FILE::AlignNumber(DWORD number, DWORD alignment)
{
    return (DWORD)(ceil(number / (alignment + 0.0)) * alignment);
}

DWORD PE64FILE::RvaToOffset(DWORD rva) {
    for (int i = 0; i < m_ntHeaders.FileHeader.NumberOfSections; i++)
    {
...
DWORD PE64FILE::OffsetToRVA(DWORD offset)
{
    for (int i = 0; i < m_ntHeaders.FileHeader.NumberOfSections; i++)
    {
...
void PE64FILE::ComputeReservedData()
{
    DWORD dirIndex = 0;
...
void PE64FILE::ComputeSectionTable()
{
