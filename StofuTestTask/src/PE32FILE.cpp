...
PEResult PE32FILE::LoadFromFile(int8_t* m_loadedPeFilePtr) {
    UnloadFile();
    m_loadedPeFile = m_loadedPeFilePtr;
...
PEResult PE32FILE::SaveToFile(std::filesystem::path filePath)
{
    Commit();
    RebuildImportTable();
    ...
    file.write((char*)&m_ntHeaders, sizeof(IMAGE_NT_HEADERS32));
...
int32_t PE32FILE::AddSection(std::string_view name, DWORD size, bool isExecutable)
{
...
void PE32FILE::AddImport(std::string_view dllName, char** functions, int functionCount)
{
...
void PE32FILE::PrintImportTable()
{
    PE_IMPORT_DLL_ENTRY* importDll = &m_importTable;

    while (importDll != nullptr)
    {
        std::cout << "DLL Name: " << importDll->Name << std::endl;

        PE_IMPORT_FUNCTION_ENTRY* importFunction = importDll->Functions;
        while (importFunction != nullptr)
        {
            if (importFunction->Id != 0)
            {
                std::cout << "  Ordinal: " << importFunction->Id << std::endl;
            }
            else
            {
                std::cout << "  Function Name: " << importFunction->Name << std::endl;
            }
...
PEResult PE32FILE::ReadHeaders()
{
    ...
    // Read PE headers
    memcpy(&m_ntHeaders, m_loadedPeFile + m_dosHeader.e_lfanew, sizeof(IMAGE_NT_HEADERS32));
    ...
    memcpy(m_sectionTable, m_loadedPeFile + m_dosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS32), m_ntHeaders.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER));

    return PEResult::Success;
}

void PE32FILE::ReadSections()
{
    DWORD reservedDataOffset = m_dosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS32) + m_ntHeaders.FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER);
...
PEResult PE32FILE::ReadImports()
{
    ...
    IMAGE_THUNK_DATA32* importThunk;
    ...
    while (true)
    {
        ...
        {
            importThunk = (IMAGE_THUNK_DATA32*)(m_loadedPeFile + RvaToOffset(importDesc->OriginalFirstThunk));
        }
        else
        {
            importThunk = (IMAGE_THUNK_DATA32*)(m_loadedPeFile + RvaToOffset(importDesc->FirstThunk));
        }
        ...
        while (true)
        {
            ...
            importThunk = (IMAGE_THUNK_DATA32*)((char*)importThunk + sizeof(IMAGE_THUNK_DATA32));
...
void PE32FILE::RebuildImportTable()
{
...
char* PE32FILE::BuildAdditionalImports(DWORD baseRVA)
{
    Commit();
    ...
    while (importDll != nullptr)
    {
        ...
        while (importFunction != nullptr)
        {
            memset(&importThunk, 0, sizeof(IMAGE_THUNK_DATA32));
            if (importFunction->Id != 0)
            {
            ...
            memcpy(buffer + offsetFunctions, &importThunk, sizeof(IMAGE_THUNK_DATA32));
            memcpy(buffer + offsetFunctions + sizeFunctions, &importThunk, sizeof(IMAGE_THUNK_DATA32));
            offsetFunctions += sizeof(IMAGE_THUNK_DATA32);
...
DWORD PE32FILE::CalculateAdditionalImportsSize(DWORD& sizeDlls, DWORD& sizeFunctions, DWORD& sizeStrings)
{
    PE_IMPORT_DLL_ENTRY* importDll = &this->m_additionalImports;
    PE_IMPORT_FUNCTION_ENTRY* importFunction;
    ...
    while (importDll != nullptr)
    {
        ...
        while (importFunction != nullptr)
        {
            sizeFunctions += sizeof(IMAGE_THUNK_DATA32);
            if (importFunction->Id == 0)
            {
...
bool PE32FILE::WritePadding(std::ofstream& file, long paddingSize)
{
    if (paddingSize <= 0)
        return false;
...
DWORD PE32FILE::AlignNumber(DWORD number, DWORD alignment)
{
    return (DWORD)(ceil(number / (alignment + 0.0)) * alignment);
}

DWORD PE32FILE::RvaToOffset(DWORD rva) {
    for (int i = 0; i < m_ntHeaders.FileHeader.NumberOfSections; i++)
    {
...
DWORD PE32FILE::OffsetToRVA(DWORD offset)
{
    for (int i = 0; i < m_ntHeaders.FileHeader.NumberOfSections; i++)
    {
...
void PE32FILE::ComputeReservedData()
{
    DWORD dirIndex = 0;
...
void PE32FILE::ComputeSectionTable()
{
